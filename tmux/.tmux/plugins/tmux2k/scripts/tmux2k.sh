...
main() {
    ...
    *)
    ...
    if $show_border_contrast; then
        tmux set-option -g pane-active-border-style "bold fg=${cyan}"
    else
        tmux set-option -g pane-active-border-style "fg=${light_purple}"
    fi
    tmux set-option -g pane-border-style "bold fg=${cyan}"
    ...
    # Status left
    if $show_powerline; then
        tmux set-option -g status-left "#[bg=${gray},fg=${magenta}]#{?client_prefix,#[bg=${yellow}} ${left_icon} #[fg=${green},mmagentaagreen}]#{?client_prefix,#[fg=${yellow}}${left_sep}"
        powerbg=${gray}
    else
        tmux set-option -g status-left "#[bg=${dark_gray},fg=${green}]#{?client_prefix,#[bg=${yellow}],} ${left_icon}"
    fi

    for lplugin in "${lplugins[@]}"; do

        if [ "$lplugin" = "git" ]; then
            IFS=' ' read -r -a colors <<<"$(get_tmux_option '@tmux2k-git-colors' 'dark_gray green')"
        ...
        if [ "$lplugin" = "ram-usage" ]; then
            IFS=' ' read -r -a colors <<<"$(get_tmux_option '@tmux2k-ram-usage-colors' 'dark_gray yellow')"
        ...
        if [ "$lplugin" = "weather" ]; then
            ...
            IFS=' ' read -r -a colors <<<"$(get_tmux_option '@tmux2k-weather-colors' 'red gray')"
            script="#(cat $datafile)"
        fi
    ...
    # Window option
    if $show_powerline; then
        tmux set-window-option -g window-status-current-format "#[fg=${gray},bg=${cyan}]${left_win_sep}#[bg=${gray}]${current_flags}#[fg=${cyan}] #I:#W #[fg=${cyan},bg=${gray}]${right_win_sep}"
    else
        tmux set-window-option -g window-status-current-format "#[fg=${white},bg=${blue}] #I:#W ${current_flags} "
    fi

    if $show_powerline; then
        tmux set-window-option -g window-status-format "#[fg=${gray},bg=${light_gray}]${left_win_sep}#[bg=${gray}]${flags}#[fg=${white}] #I:#W #[fg=${light_gray},bg=${gray}]${right_win_sep}"
    else
    ...
    for rplugin in "${rplugins[@]}"; do
        ...
        if [ "$rplugin" = "git" ]; then
            IFS=' ' read -r -a colors <<<"$(get_tmux_option '@tmux2k-git-colors' 'green black')"
            script="#($current_dir/git.sh)"
        fi
        ...
        if [ "$rplugin" = "network-bandwidth" ]; then
            IFS=' ' read -r -a colors <<<"$(get_tmux_option '@tmux2k-network-bandwidth-colors' 'red gray')"
            tmux set-option -g status-right-length 250
            script="#($current_dir/network_bandwidth.sh)"
        fi

        if [ "$rplugin" = "network-ping" ]; then
            IFS=' ' read -r -a colors <<<"$(get_tmux_option '@tmux2k-network-ping-colors' 'red gray')"
            script="#($current_dir/network_ping.sh)"
        fi
